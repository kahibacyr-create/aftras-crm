...
export const RemoteFormView: React.FC<{ user: UserApp }> = ({ user }) => {
  ...
  // Fonction de simulation pour démontrer le workflow et les notifications
  const simulateIncomingLead = async () => {
    const names = ["Amadou Diallo", "Fanta Camara", "Moussa Sow", "Awa Diop", "Bakary Konaté"];
    ...
    await dataService.addRemoteLead({
      ...
      details: "Ceci est un lead de test généré par le bouton simulation."
    });
  };
      ...
      <div className="bg-white p-12 rounded-[4rem] border border-gray-100 shadow-2xl relative overflow-hidden text-center group">
        ...
        <div className="mt-12 flex flex-col sm:flex-row gap-4 items-center justify-center">
          ...
          <div className="flex gap-2">
            <button 
              onClick={() => { navigator.clipboard.writeText(link); alert('Lien copié avec succès !'); }}
            ...
            <button 
              ...
              title="Générer un lead de test"
            >
              <span className="group-hover/sim:animate-pulse">⚡ Simuler</span>
            </button>
          </div>
        </div>
      </div>
